    ...
    const initializePublicData = async () => {
        ...
        const unsubscribeLogo = onSnapshot(qLogo, (snapshot) => {
            if (!snapshot.empty) {
                ...
                if (data.logoDataUri) {
                    let rawUri = data.logoDataUri;
                    // Smartly extract the data URI if the user pasted the whole JS variable
                    const match = rawUri.match(/'(data:image[^']*)'/);
                    if (match && match[1]) {
                        setLogoDataUri(match[1]);
                    } else if (rawUri.startsWith('data:image')) {
                        setLogoDataUri(rawUri);
                    }
                }
            }
        }, (err) => {
